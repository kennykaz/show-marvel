...
function App() {
  ...
  return (
    <div className="leaderboard">
      ...
      <Modal 
        isEnabled={isTeamVisible} 
        heros={heros ? Array.from(heros.values()) : []}
        onHeroAction={onHeroAction}
        onClose={() => setIsTeamVisible(false)}
      />
    </div>
  );
}
...
function Profile(props: { 
  hero: Hero, 
  isInTeam: boolean, 
  onHeroAction: (hero: Hero, action: 'add' | 'remove') => void
}) {
  
  const { hero, isInTeam, onHeroAction } = props;
  
  return (
    <article key={hero.id} className="leaderboard__profile">
      <img src={`${hero.thumbnail.path}/standard_medium.${hero.thumbnail.extension}`} alt={hero.name} className={isInTeam ? 'leaderboard__picture selected' : 'leaderboard__picture'}/>
      <span className="leaderboard__name">{hero.name}</span>
...
function Modal(props: {
  ...
  const {isEnabled, heros, onHeroAction, onClose} = props;

  return (
    <div className={isEnabled ? "modal-window open" : "modal-window"}>
      <div>
        <Header header="Team List" subHeader="Your Marvel Team"/>
        <button className="modal-close" onClick={onClose}>Close</button>
        <main className="leaderboard__profiles">
          {heros.map(hero => <Profile key={hero.id} hero={hero} isInTeam={true} onHeroAction={onHeroAction}/>)}
        </main>
      </div>
    </div>
  )
...
function Header(props: {header: string, subHeader: string}) {
  const {header, subHeader} = props;
  return (
    <header>
        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 511.999 511.999" className="leaderboard__icon">
          <g>
            <g>
        ...
        <h1 className="leaderboard__title"><span className="leaderboard__title--top">{header}</span><span className="leaderboard__title--bottom">{subHeader}</span></h1>
      </header>
  );
}
